...
// VBUS was not properly connected and detected by the CPU
  ...
  private:
    ...
    // 2026-09-21 解耦（与 S3 树同步）：充电/USB 判定阈值不再从 OCV 首项派生 ✗
    //   本板无 USB 检测脚 ⇒ isVbusIn() 用「电压 > chargingVolt」推断外部供电；
    //   而 OCV 首项已按电池下调到 4170（满电 = 100%）⇒ 派生会把阈值拉到 4180mV，
    //   而本板电池满电静置 4.175V ⇒ 只差 5mV ⇒ 迟早误判「在充电」✗
    //   ⇒ 固定 4200mV（可用 -D CHARGING_VOLT_MV 覆盖）✓
